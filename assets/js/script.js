    ...
    try {
      ...
      if (textInputValue != ""){ //checking if text was entered
        ...
        if (textInputValueLowerCase.substr(0,5) == "open ") { //if the first 5 characters = open + space
          ...
          addTextToResults("<i>The URL " + "<b>" + textInputValue.substr(5) + "</b>" + " should be open now.</i>");
          return;
        } 
        if (textInputValueLowerCase.substr(0,8) == "youtube ") {
          openLinkInNewWindow('https://www.youtube.com/results?search_query=' + textInputValueLowerCase.substr(8));
          addTextToResults("<i>I've searched on YouTube for " + "<b>" + textInputValue.substr(8) + "</b>" + " it should be open now.</i>");
          return;
        } 
        if (textInputValueLowerCase.substr(0,7) == "google ") {
          openLinkInNewWindow('https://www.google.com/search?q=' + textInputValueLowerCase.substr(7));
          addTextToResults("<i>I've searched on Google for " + "<b>" + textInputValue.substr(7) + "</b>" + " it should be open now.</i>");
          return;
        } 
        if (textInputValueLowerCase.substr(0,5) == "wiki "){
          openLinkInNewWindow('https://wikipedia.org/w/index.php?search=' + textInputValueLowerCase.substr(5));
          addTextToResults("<i>I've searched on Wikipedia for " + "<b>" + textInputValue.substr(5) + "</b>" + " it should be open now.</i>");
          return;
        } else {
            postHelpList();
            clearInput();
